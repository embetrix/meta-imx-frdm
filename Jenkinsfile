pipeline {
    ...
    parameters {
        ...
        choice choices: ['imx91frdm', 'imx93frdm'], description: 'select machine', name: 'MACHINE'
    ...
    stages {
        ...
        stage('Build-Image') {
            steps {
                sh "KAS_MACHINE=${params.MACHINE} KAS_TARGET=${params.IMAGE} kas build --force-checkout --update kas-imx-frdm.yml"
        ...
        stage ('Build-SDK') {
            ...
            steps {
               sh "KAS_MACHINE=${params.MACHINE} KAS_TARGET=${params.IMAGE} KAS_TASK=populate_sdk kas build kas-imx-frdm.yml"
